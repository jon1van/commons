...
import static io.github.jon1van.commons.maps.MapFeatures.circle;
...
import io.github.jon1van.units.Navigation;
...
///
/// A MapImage saves its "base image" so that it can be repeatedly "drawn on" without needing to
...
public class MapImage {
    ...
    public MapImage(TileServer tileServer, LatLong center, Distance width) {
        ...
        this.topLeft = center.move(Course.WEST, halfWidth)
                .move(Course.NORTH, halfWidth)
                .latLong();
        this.bottomRight = center.move(Course.EAST, halfWidth)
                .move(Course.SOUTH, halfWidth)
                .latLong();
    ...
    private static BufferedImage buildCroppedImage(
        ...
        BufferedImage undecoratedImage = combinedTileImage.getSubimage(
                ...
                (int) Math.round(excerptBottomRight.x() - excerptTopLeft.x()),
                (int) Math.round(excerptBottomRight.y() - excerptTopLeft.y()));
    ...
    private static int computeZoomLevel(Distance width) {
        ...
        Distance currentDistance = Distance.ofNauticalMiles(2 * PI * Navigation.EARTH_RADIUS_NM);
        int zoomLevel = 0;
    ...
    public void plotToFile(File targetFile) {
        plotToFile(FeatureSet.noMapFeatures(), targetFile);
    }
    ...
    /// dark-mode map.
    ///
    /// This method is backed by a MapBuilder and some LatLong to MapFeature conversion.
    ///
    /// Consider adding a call to "writeImageToFile(mapImage, new File("quickMap.jpg"));"
    ///
    ...
    public static BufferedImage plotLocationData(List<LatLong> locations, Distance mapWidth) {
        ...
        LatLong center = avgLatLong(locations.toArray(new LatLong[0]));
