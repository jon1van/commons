...
import static io.github.jon1van.commons.maps.FeatureSetBuilder.newFeatureSetBuilder;
import static io.github.jon1van.commons.maps.MapBuilder.newMapBuilder;
...
import io.github.jon1van.units.LatLongPath;
...
class MapBuilderTest {
    ...
    public void plotMap_exactly600Pixels() {
        ...
        assertThat(newMapImage.getWidth()).isEqualTo(600);
        assertThat(newMapImage.getHeight()).isEqualTo(600);
    }
    ...
    public void plotDotsOnSolidBackground() throws Exception {
        ...
        List<LatLong> blueDots = Stream.of(
                        ...
                        LatLong.of(32.8968, -97.0380).move(EAST, Distance.ofMiles(1)),
                        LatLong.of(32.8968, -97.0380).move(EAST, Distance.ofMiles(2)),
                        LatLong.of(32.8968, -97.0380).move(EAST, Distance.ofMiles(3)),
                        LatLong.of(32.8968, -97.0380).move(EAST, Distance.ofMiles(4)))
                .collect(toList());

        // render the LatLong as one group
        List<LatLong> redDots = Stream.of(
                        LatLong.of(32.8968, -97.0380),
                        LatLong.of(32.8968, -97.0380).move(NORTH, Distance.ofMiles(1)),
                        LatLong.of(32.8968, -97.0380).move(NORTH, Distance.ofMiles(2)),
                        LatLong.of(32.8968, -97.0380).move(NORTH, Distance.ofMiles(3)),
                        LatLong.of(32.8968, -97.0380).move(NORTH, Distance.ofMiles(4)))
                .collect(toList());
    ...
    private void verifyImagesMatch(BufferedImage img1, BufferedImage img2) {
        ...
        for (int i = 0; i < img1.getWidth(); i++) {
            for (int j = 0; j < img1.getHeight(); j++) {
                assertThat(img1.getRGB(i, j)).isEqualTo(img2.getRGB(i, j));
            }
        }
    }
    ...
    List<LatLong> randomWalk(LatLong start) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            LatLong next = list.getLast().move(direction, Distance.ofNauticalMiles(0.05));
            list.add(next);
        }
    ...
    //                        datum,
    //                        new Position(datum.time(), LatLong.of(datum.latitude(), datum.longitude()),
    // datum.altitude())))
    //                .collect(toList());
    //
    ...
    //
    //        newMapBuilder()
    //                //            .mapBoxDarkMode()  //this requires having the Mapbox secret available (which might
    // not
    //                // work in CI)
    ...
    void canMakeMapWithCirclesPathsAndText() {
        ...
        newMapBuilder()
                ...
                .center(home)
                .width(Distance.ofNauticalMiles(10))
                .addFeature(circles(path, Color.BLUE, 22, 11.0f))
                .addFeature(path(path, Color.BLUE, 11.0f))
