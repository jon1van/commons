...
public class Navigation {
    ...
    ///
    /// @return The course between two points in degrees
    ...
    /// @param startLon longitude in degrees (of point 1)
    /// @param endLat    latitude in degrees (of point 2)
    /// @param endLon   longitude in degrees (of point 2)
    ///
    ...
    /// Compute a new location from a starting location, a direction, and a distance.
    ///
    ...
    public static LatLong128 move(double latDeg, double lonDeg, double headingDegree, double distNM) {
    ...
    /// Compute a new LatLong location from a starting location, a direction, and a distance.
    ///
    ...
    public static LatLong move(HasLatLong start, Course direction, Distance distance) {
        return Navigation.move(start.latitude(), start.longitude(), direction.inDegrees(), distance.inNauticalMiles())
                .latLong();
    }

    /// Compute a new LatLong location from a starting location, a direction, a distance, and a
    /// curvature.
    ///
    /// @param start     The starting location
    /// @param direction The direction of travel
    ...
    ///
    /// @return The destination location
    public static LatLong move(HasLatLong start, Course direction, Distance distance, double curvature) {
        return move(start.latitude(), start.longitude(), direction.inDegrees(), distance.inNauticalMiles(), curvature)
                .latLong();
    }
    ...
    ///
    /// @return The destination point
    public static LatLong128 move(double latDeg, double lonDeg, double headingDegree, double distNM, double curvature) {
        double rMax = EARTH_RADIUS_NM * Math.PI / 2.0;
        double radiusNM = Math.max(-rMax, Math.min(rMax, 1.0 / curvature));
        LatLong128 pair = Navigation.move(latDeg, lonDeg, mod(headingDegree + 90.0, 360.0), radiusNM);
        ...
        return Navigation.move(latCen, lonCen, crsCen, Math.abs(radiusNM));
    }
    ...
    public static LatLong128 greatCircleOrigin(Double latitude, Double longitude, Double course) {
        return Navigation.move(
                latitude, longitude, Navigation.mod(course + 90.0, 360.0), oneQuarterCircumferenceOfEarthInNM());
    }
    ...
    public static Double distanceInNM(HasLatLong pt1, HasLatLong pt2) {
    ...
    //     */
    //    public static double alongTrackDistanceNM(HasPosition startPoint, HasPosition endPoint, HasPosition p, double
    // CTD) {
    //        double relAng = Spherical.angleDifference(startPoint.courseInDegrees(endPoint),
    ...
    //        // @todo: Replace line with `ratio = constrainToRange(ratio, -1.0, 1.0);` when Guava is shaded from
    // exported
    //        // library
    //
