...
import static io.github.jon1van.units.Course.Unit.DEGREES;
import static io.github.jon1van.units.Course.Unit.RADIANS;
...
public record Course(double angle, Unit unit) implements Comparable<Course> {
    ...
    /// @param unit  The Course Unit (i.e. DEGREES or RADIANS)
    public Course {
    ...
    public static Course angleBetween(Course one, Course two) {
        return Course.ofDegrees(Navigation.angleDifference(one.inDegrees(), two.inDegrees()));
    }
    ...
    public int hashCode() {
        int hash = 7;
        hash = 73 * hash + Long.hashCode(Double.doubleToLongBits(this.angle));
