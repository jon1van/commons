...
class ReservoirSamplerTest {
    ...
    void demoHappyPath() {
        ...
        assertThat(sampler.countSeen()).isEqualTo(7);
        assertThat(sampler.currentSample().size()).isEqualTo(2);
        assertThat(letters.containsAll(sampler.currentSample())).isTrue();
    }
    ...
    void samplesListCanBeShorterThanSampleSize() {
        ...
        assertThat(sampler.countSeen()).isEqualTo(2);
        assertThat(samples.size()).isEqualTo(2);

        assertThat(sampler.sampleSize()).isEqualTo(7); // note the subtle tension
    }
    ...
    void samplesHaveNoObviousFlaw() {
        ...
        assertThat(sum).isEqualTo(NUM_TRIALS * SAMPLE_SIZE);

        // every number is sampled more than 5 times (expected sample count = 20) (leaving room for random variance)
        Arrays.stream(sampleCounts).forEach(sample -> assertThat(sample > 5).isTrue());
    }
    ...
    public void samplingIsStatisticallySound() {
        ...
        for (int sampleCount : sampleCounts) {
            ...
            assertThat(abs(sampleCount - 5000)).isLessThan(3 * 50);
        }
    }
}
