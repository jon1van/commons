...
import static io.github.jon1van.math.CurveFitters.weightedFit;
...
import io.github.jon1van.math.locationfit.LatLongFitter.KineticValues;
...
///
/// Generally, a LocalPolyInterpolator is used repeatedly to manufacture many noise-reduced
/// KineticPositions.
///
/// A LocalPolyInterpolator can be configured to ignore altitude data. Ignoring altitude data
/// will increase computational speed (because less work is done) and allow the input Positions to
/// not have altitude data.
///
/// NOTE: The polynomial fitting process has a stricter requirement than just "N input samples".
...
public class LocalPolyInterpolator implements PositionInterpolator {
    ...
    ///                   "interpolate" method.
    ///
    /// Note: this windowSize should be thought of as a "diameter" not a
    ///                   "radius".
    ...
    /// @param requiredPoints The number of PositionRecords that are required to generate
    ///                       KineticRecords
    /// @param ignoreAltitude Do not interact with altitude data in any way.
