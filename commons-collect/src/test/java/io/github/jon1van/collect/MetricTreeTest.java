...
public class MetricTreeTest {
    ...
    public void testDuplicatePutInput() {
        ...
        assertThat(beforeAnyPuts).isNull(); // The 1st call to put should return null

        String shouldBeFirst = tree.put(new Point(0, 0), "secondItem");
        assertThat(shouldBeFirst).isEqualTo("firstItem"); // Put should return the value being replaced

        String shouldBeSecond = tree.put(new Point(0, 0), "thirdItem");
        assertThat(shouldBeSecond).isEqualTo("secondItem"); // Put should return the value being replaced

        assertThat(tree.size()).isEqualTo(1); // Repeatedly using the same Key should leave only 1 item in the tree
    ...
    public void testContainsKeyAndRemove() {
        ...
        for (Map.Entry<Point, String> entry : map.entrySet()) {
            ...
            assertThat(tree.containsKey(entry.getKey())).isTrue();
            ...
            if (rng.nextBoolean()) {
                ...
                assertThat(tree.containsKey(entry.getKey())).isFalse();
                assertThat(prior).isEqualTo(entry.getValue());
            }
        }
    }
    ...
    private void confirmExactMatchWithTestData(MetricTree<Point, String> testTree) {
        ...
        for (Map.Entry<Point, String> entry : map.entrySet()) {
            ...
            assertThat(testTree.containsKey(entry.getKey())).isTrue();

            // the testTree contents should match the testMap contents
            assertThat(testTree.get(entry.getKey())).isEqualTo(entry.getValue());

            String prior = testTree.remove(entry.getKey());

            assertThat(entry.getValue()).isEqualTo(prior);
            assertThat(testTree.containsKey(entry.getKey())).isFalse();
        }

        // tree should be empty by now
        assertThat(testTree.size()).isEqualTo(0);
        assertThat(testTree.isEmpty()).isTrue();
    }
