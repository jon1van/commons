...
public class ManyKeysWithSameLocationTest {
    ...
    public void testTooManyIdenticalKeys() {
        ...
        assertThat(tree.size() == INITIAL_SIZE).isTrue();
    ...
    private void confirmEqualContent(MetricTree<Location, String> tree, HashMap<Location, String> copyOfTreeContent) {
        ...
        assertThat(tree.size() == copyOfTreeContent.size()).isTrue();

        SetView<Location> keyIntersection = Sets.intersection(tree.keySet(), copyOfTreeContent.keySet());

        // If the intersection between these two sets is the same size as one of the orginal sets then the sets are
        // equal
        assertThat(tree.size() == keyIntersection.size()).isTrue();
