...
import static io.github.jon1van.commons.ids.SmallTimeId.NUM_BITS_FOR_DISTINGUISHING_ITEMS;
...
import io.github.jon1van.commons.ids.TimeIds.IdFactory;

/// A [IdFactoryShard] is one member of a "team of non-communicating factories" that
...
///
/// The SmallTimeIds bits are:
///
/// {{42 time bits}} + {{n shardIndex bits }} + {{ (21-n) shard-specific counter bits}}
///
/// A [IdFactoryShard] is essentially a Snowflake_IDs implementation (see
/// https://en.wikipedia.org/wiki/Snowflake_ID) that does not include the REST API layer.
///
/// The implementation differences are: Snowflake_ID uses 41 bits for the timestamp, Snowflake_ID
...
///
/// Philosophically, [IdFactoryShard] is designed to:
///
///  - 1: Make more efficient use of the available bitspace by avoiding the pitfalls of hashing
...
///
///  - 2: Support non-colliding distributed key-generation (and parallelism) by partitioning the
...
public class IdFactoryShard implements IdFactory<SmallTimeId> {
    ...
    ///
    /// But, the "REAL GOAL" of CountKeeper is to provide a seam where important but subtle
    ...
    ///
    /// Consequently, when this CountKeeper "runs out of available Instant counters" it will
    ...
    /// Instant".
    ///
    /// The "counter eviction" policy requires this CountKeeper to receive Instant in roughly
    ...
    /// still enforceable.
    ///
    /// Thus, when a CappedInMemoryCountKeeper "runs out of Instant counters" it will start
